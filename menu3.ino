...
const int D3 = 3; // pompa main - zbiornik wyrównawczy
const int D4 = 4;
const int D5 = 5;
const int D6 = 6;
const int stateOfMainZbiornik = 7; // stan powiomu w zbiorniku wyrównawczym
...
int buttonZbiornikState;             // the current reading from the input pin
...
int lastButtonZbiornikState = LOW;
...
unsigned long lastDebounceZbiornikTime = 0;  // the last time the output pin was toggled
...
int executionValve1Main = LOW;
int executionDescription = LOW;
...
long timerValveMain1On = 10000;
long timers[] = { timerPumpOn, timerPumpOff, timerValve1On, timerValve1Off, timerValve2On, timerValve2Off };
long timerDescription = 3000;

int eeAddressStart1 = 10;
int eeAddressStart5 = 50;
...
int bottleSize = 1;
long numbersOfLiters = 0;

unsigned long lastMillisTimeFill = 0;
unsigned long lastMillisTimeFillDescryption = 0;
unsigned long lastMillisTimeUnFillZbionik = 0;
...
void setup() {
  ...
  Indio.digitalMode(stateOfMainZbiornik, INPUT);
  ...
  Indio.digitalWrite(D4,LOW);

  lcd.begin();  //sets the resolution of the LCD screen

  EEPROM.get(1, numbersOfLiters);
  lcd.clear();
  lcd.setCursor(10, 2); //set the cursor to the fifth pixel from the left edge, third row.
  lcd.print("Inicjalization...");
  lcd.setCursor(10, 3); //set the cursor to the fifth pixel from the left edge, third row.
  lcd.print(numbersOfLiters);
  delay(3000);
...
void loop() {
  ...
  if (buttonExternalPress == HIGH){
    ...
    if (runFillingBottles == LOW){
      ...
      if (bottleSize == 1){
        numbersOfLiters = numbersOfLiters + 2;
        EEPROM.put(1, numbersOfLiters);
      }
      if (bottleSize == 5){
        numbersOfLiters = numbersOfLiters + 10;
        EEPROM.put(1, numbersOfLiters);
      }
  ...
  if (buttonEnterPress == HIGH){
      ...
      if (bottleSize == 1){
        ...
        */
        for (int i = 0; i <= menuPositionMax; i++){
          EEPROM.get(eeAddressStart5 + (i * 4), timers[i]);
        }
        ...
        */
        for (int i = 0; i <= menuPositionMax; i++){
          EEPROM.get(eeAddressStart1 + (i * 4), timers[i]);
        }
        printTimers();
      }
    }
    ...
    if (menuPosition == 6){
      ...
      if (bottleSize == 1){
        for (int i = 0; i <= menuPositionMax; i++){
          EEPROM.put(eeAddressStart1 + (i * 4), timers[i]);
        }
        executionDescription = 1;
        lastMillisTimeFillDescryption = millis();
        lcd.setCursor(7, 7);
        lcd.print("Save - b.1L");
      }
      else if (bottleSize == 5){
        for (int i = 0; i <= menuPositionMax; i++){
          EEPROM.put(eeAddressStart5 + (i * 4), timers[i]);
        }
        executionDescription = 1;
        lastMillisTimeFillDescryption = millis();
        lcd.setCursor(7, 7);
  ...
  if (executionDescription == 1 && (millis() - lastMillisTimeFillDescryption) >= timerDescription){
    executionDescription = 0;
    lcd.setCursor(0, 7);
    lcd.print("                                      ");
  }

  if (executionValve1Main == 1 && (millis() - lastMillisTimeUnFillZbionik) >= timerValveMain1On){
    executionValve1Main = 0;
    Indio.digitalWrite(D3,HIGH);
  }
...
void readButtons() {
  ...
  int readingZbiornik = Indio.digitalRead(stateOfMainZbiornik);
...
/*  if (readingZbiornik != lastButtonZbiornikState) {
    // reset the debouncing timer
    lastDebounceZbiornikTime = millis();
    //lcd.print("y");
  }*/
...
/*  if ((millis() - lastDebounceZbiornikTime) > debounceDelay) {
    if (readingZbiornik != buttonZbiornikState) {
      Serial.println("Valve Main - #1");
      buttonZbiornikState = readingZbiornik;
      if (buttonZbiornikState == HIGH) {
        buttonZbiornikPress = HIGH;
        Serial.println("Valve Main - #2");
      }
    }
  }
*/
  if (readingZbiornik == HIGH) {
      //Serial.println("HIGH");
      Indio.digitalWrite(D4,LOW);
  }

  if (readingZbiornik == LOW) {
      //Serial.println("HIGH");
      Indio.digitalWrite(D4,HIGH);
  }
